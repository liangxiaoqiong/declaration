 ...
 */
class DataApply2018Model extends BaseModel implements Action
{
    protected $tableName = "data_apply_2018";
     ...
     * @return array
     * User: hjun
     * Date: 2019-02-28 20:35:42
     * Update: 2019-02-28 20:35:42
     * Version: 1.00
     */
    public function getDataApply2018($companyId = 0)
    {
        $where = [];
    ...
    {
        $info = $this->getLastQueryData("data_apply_2018_{$dataId}");
        if (empty($info)) {
            $info = $this->getDataApply($dataId);
            $this->setLastQueryData("data_apply_2018-{$dataId}", $info);
        }
        return $info;
    }
    ...
    public function validateCanAddDataApply()
    {
        ...
        // 已提交过不能再提交
        if ($this->is2018DataApply($company)) {
            $this->setValidateError('已提交数据申报,无需再提交');
            return false;
        }
        return true;
    }
    ...
    {
        return strtotime('2018-01-01 00:00:00');
    }
    ...
    public function action($action = DATA_ACTION_ADD, $request = [])
    {
        ...
        if ($type == self::MODEL_INSERT) {
            ...
            $companyData = [];
            $companyData['data_apply_2018_status'] = COMPANY_DATA_APPLY_STATUS_YES;
            $companyData['data_apply_2018_time'] = $data['apply_time'];
            ...
            if ($action === DATA_ACTION_REFUSE) {
                // 如果是拒绝 企业设置为未填报
                $where = [];
                $where['company_id'] = $oldData['company_id'];
                $companyData = [];
                $companyData['data_apply_2018_status'] = COMPANY_DATA_APPLY_STATUS_NO;
                $results[] = D('Company')->where($where)->save($companyData);
            } elseif ($action === DATA_ACTION_SAVE) {
                // 如果是拒绝 企业设置为已填报
                $where = [];
                $where['company_id'] = $oldData['company_id'];
                $companyData = [];
        ...
        $results[] = D('DataApplyLog2018')->addLog($action, $oldData, $data, $request, $desc);
