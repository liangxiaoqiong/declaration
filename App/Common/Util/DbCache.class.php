...
class DbCache extends File
{
    ...
    private $tableCacheMapType; // 0-数据库 1-数组
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->setNoCacheTable(['wjd_db_cache', 'wjd_admin_log']);
    }
    ...
    public function afterChange($action = self::INSERT_ACTION, $data, $options)
    {
        ...
        $tables = SQL::$method();
        // 企业缓存清空
        if (in_array('imis_company', $tables)) {
            if (isset($options['where']['company_id'])) {
                $companyId = $options['where']['company_id'];
                S("company_{$companyId}", null);
            }
        }

        // 问卷缓存清空
        if (in_array('imis_qnr', $tables)) {
            if (isset($options['where']['qnr_id'])) {
                $qnrId = $options['where']['qnr_id'];
                S("qnr_{$qnrId}", null);
            }
        }
    ...
    public function saveQueryCacheMap($set)
    {
        ...
        return $this->isDbType() ? false : $this->finish();
    }
